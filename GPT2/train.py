...
def setup_train_args():
    ...
    parser.add_argument('--model_config', default='config/model_config_dialogue_small.json', type=str, required=False,
                        help='选择模型参数')
    parser.add_argument('--vocab_path', default='vocabulary/vocab_se.txt', type=str, required=False, help='选择词库')
    parser.add_argument('--train_raw_path', default='data_ocr/train.txt', type=str, required=False, help='原始训练语料')
    parser.add_argument('--train_tokenized_path', default='data_ocr/all_tokenized.txt', type=str,
                        required=False,
                        help='将原始训练语料tokenize之后的数据的存放位置')
    parser.add_argument('--log_path', default='data_ocr/training.log', type=str, required=False, help='训练日志存放位置')
    ...
    parser.add_argument('--lr', default=3e-4, type=float, required=False, help='学习率')
    ...
    parser.add_argument('--dialogue_model_output_path', default='dialogue_pos_model/', type=str, required=False,
                        help='对话模型输出路径')
    ...
    parser.add_argument('--train_mmi_tokenized_path', default='data_ocr/all_mmi_tokenized.txt', type=str,
                        required=False,
                        help='将原始训练语料的每段对话翻转，然后进行tokenize之后的数据的存放位置，用于训练MMI模型')
    parser.add_argument('--start_point', type=int, required=False, default=0)
    parser.add_argument('--mmi_model_output_path', default='mmi_pos_model/', type=str, required=False, help='MMI模型保存路径')
    parser.add_argument('--n_ctx', type=int, default=1024, help='最长序列长度')
...
def create_model(args, vocab_size):
    ...
    if args.pretrained_model:  # 如果指定了预训练的GPT2模型
        ...
        pattern = 'epoch(\d+)'
        args.start_point = int(re.findall(pattern, args.pretrained_model)[0])
...
def calculate_loss_and_accuracy_without_ocr(outputs, labels, token_type_ids, device):
    """
    计算非pad_id的平均loss和准确率
    :param outputs:
    :param labels:
    ...
    # 用第i个token的prediction_score用来预测第i+1个token。
    # 假定有input有n个token，则shift_logits表示model中第[0,n-2]个token的prediction_score，shift_labels表示第[1，n-1]的label
    shift_logits_ = logits[..., :-1, :].contiguous()
    shift_labels_ = labels[..., 1:].contiguous().to(device)
    shift_token_type_ = token_type_ids[..., 1:].contiguous()

    shift_logits_ = shift_logits_.view(-1, shift_logits_.size(-1))
    shift_labels_ = shift_labels_.view(-1)
    shift_token_type_ = shift_token_type_.view(-1)
    indices = (shift_token_type_ != 2) & (shift_token_type_ != 3)
    shift_logits = shift_logits_[indices]
    shift_labels = shift_labels_[indices]
...
def collate_fn(batch):
    ...
    for btc_idx in range(btc_size):
        ...
        token_pad_id = batch[btc_idx]['token_type_ids'][-1]
        token_type_ids[btc_idx].extend([token_pad_id] * (max_input_len - input_len))

        position_ids.append(batch[btc_idx]['position_ids'])
        position_ids[btc_idx].extend([1023] * (max_input_len - input_len))

        turn_ids.append(batch[btc_idx]['turn_ids'])
        turn_ids[btc_idx].extend([59] * (max_input_len - input_len))
...
def train(model, device, train_list, multi_gpu, args):
    print(args.n_ctx)
    train_dataset = MyDataset(train_list, args.n_ctx, args.train_mmi)
    ...
    oom_time = 0
    # 开始训练
    for epoch in range(args.start_point, args.epochs):
        ...
        for batch_idx, batch in enumerate(tqdm(train_dataloader)):
            ...
            try:
                ...
                outputs, loss, accuracy = model.forward(input_ids=input_ids, token_type_ids=token_type_ids, position_ids=position_ids, turn_ids=turn_ids)
                #loss, accuracy = calculate_loss_and_accuracy_without_ocr(outputs, labels=input_ids, token_type_ids=token_type_ids, device=device)
...
def evaluate(model, device, test_list, multi_gpu, args):
    ...
    # 记录tensorboardX
    #tb_writer = SummaryWriter(log_dir=args.writer_dir)
    test_dataset = MyDataset(test_list, args.n_ctx, args.train_mmi)

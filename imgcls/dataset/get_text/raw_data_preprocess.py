...
def gen_train_dev_set(dir, train_items, dev_items):
    #f_train_out = open('./data/train' + '.txt', 'w')
    #f_dev_out = open(dir + 'dev' + '.txt', 'w')
    #f_dev_out = open('./data/dev.txt', 'w')
    text_for_img = {}
    ...
    for type in ['train', 'dev']:
        ...
        for i, item in enumerate(tqdm(items)):
            ...
            trg_str = trg_str.strip().replace(' ', '')
            #img_str = ' '.join(img_list)
            for img in img_list:
                if img not in text_for_img.keys():
                    text_for_img[img] = ques.strip().replace('<img>', '')
...
if __name__ == '__main__':
    ...
    parser.add_argument('-d', '--directory', default='/raid/zxy/project/jddc/data/')
    parser.add_argument('-s', '--sess_turns', default=10)

    args = parser.parse_args()

    train_items, dev_items = do_preprocess(args.directory, args.sess_turns)

    text_for_img = gen_train_dev_set(args.directory, train_items, dev_items)
    print('text_for_img done')
    text_for_label = {}
    text_txt = open('./text_ques.txt', 'w')
    with open('./tmp.txt', 'rb') as f:
        lines = f.read().decode('utf-8')
    for line in lines.split('\n')[:-1]:
        ls = line.split('\t')
        if ls[0] not in text_for_img.keys():
            continue
        if ls[1] not in text_for_label.keys():
            text_for_label[ls[1]] = text_for_img[ls[0]]
        else :
            text_for_label[ls[1]] = text_for_label[ls[1]] +  text_for_img[ls[0]]
    print('text_for_label done')
    for label in tqdm(text_for_label.keys()):
        wordsls=jieba.lcut(text_for_label[label])
        wcdict={}
        for word in wordsls:
            if len(word)==1:
                continue

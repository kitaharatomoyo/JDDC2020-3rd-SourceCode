...
def train():
    ...
    for epoch in range(config.num_epochs):
        ...
        for i, (ims, label) in enumerate(dataloader_train):
            ...
            top1, _, _ = accuracy(output.data, target.data, topk=(1,))
        ...
        if epoch % 10 == 0 and epoch < config.num_epochs:
            # eval
            val_time_start = time.time()
            val_loss, val_top1, val_recall, val_f1 = eval(model, dataloader_valid, criterion)
            val_draw_acc.append(val_top1)
            val_time = (time.time() - val_time_start) / 60.

            print('Epoch [%d/%d], Val_Loss: %.4f, Val_top1: %.4f, Val_recall: %.4f, Val_f1: %.4f, val_time: %.4f s'
                   %(epoch+1, config.num_epochs, val_loss, val_top1, val_recall, val_f1, val_time*60))
            print('epoch time: {}s'.format(epoch_time*60))
            if val_top1[0].data > max_val_acc:
                max_val_acc = val_top1[0].data
                print('Taking snapshot...')
                if not os.path.exists('./checkpoints/ocr'):
                    os.makedirs('./checkpoints/ocr')
                torch.save(model, '{}/{}/{}.pth'.format('checkpoints', 'ocr', config.model))

            log.write('Epoch [%d/%d], Val_Loss: %.4f, Val_top1: %.4f, Val_recall: %.4f, Val_f1: %.4f, val_time: %.4f s\n'
                       %(epoch+1, config.num_epochs, val_loss, val_top1, val_recall, val_f1, val_time*60))
...
def eval(model, dataloader_valid, criterion):
    ...
    for ims, label in dataloader_valid:
        ...
        top1_val, recall, f1 = accuracy(output_val.data, target_val.data, topk=(1,))
    ...
    return avg_loss, avg_top1, avg_recall, avg_f1
